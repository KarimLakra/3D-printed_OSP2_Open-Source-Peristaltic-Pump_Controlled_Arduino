...
void UTFT::fillRect(int x1, int y1, int x2, int y2)
{
	...
	if (display_transfer_mode==16)
	{
		...
		//_fast_fill_16(fch ^ invCol, fcl ^ invCol, ((long(x2 - x1) + 1)*(long(y2 - y1) + 1)));//Kardis
		sbi(P_CS, B_CS);
	}
...
void UTFT::setColor(byte r, byte g, byte b)
{
	fch=((r&248)|g>>5);
	fcl=((g&28)<<3|b>>3);
	//fch = ((r & 248) | g >> 5) ^ invCol;//Kardis
	//fcl = ((g & 28) << 3 | b >> 3) ^ invCol;//Kardis
}
...
void UTFT::setColor(word color)
{
	fch=byte(color>>8);
	fcl=byte(color & 0xFF);
	//fch = byte(color >> 8) ^ invCol;//Kardis
	//fcl = byte(color & 0xFF) ^ invCol;//Kardis
}

word UTFT::getColor()
{
	return (fch<<8) | fcl;
	//return ((fch << 8) | fcl) ^ invCol;//Kardis
}
...
void UTFT::setBackColor(byte r, byte g, byte b)
{
	bch=((r&248)|g>>5);
	bcl=((g&28)<<3|b>>3);
	//bch = ((r & 248) | g >> 5) ^ invCol;//Kardis
	//bcl = ((g & 28) << 3 | b >> 3) ^ invCol;//Kardis
	_transparent = false;
}
...
void UTFT::setBackColor(uint32_t color)
{
	...
	else
	{
		//bch=byte(color>>8);
		//bcl=byte(color & 0xFF);
		bch = byte(color >> 8) ^ invCol;//Kardis
		bcl = byte(color & 0xFF) ^ invCol;//Kardis
		_transparent=false;
	}
}
...
{
	//return (bch<<8) | bcl;
	return ((bch << 8) | bcl) ^ invCol;//Kardis
}
